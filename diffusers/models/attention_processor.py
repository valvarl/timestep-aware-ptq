...
class FluxAttnProcessor2_0:
    ...
    def initialize_fake_quantization(self, attn):
        for linear_name in ("to_q", "to_k", "to_v"):
            linear = getattr(attn, linear_name)
            ...
            # set the weight and bias of the fake quantization layers to the weight and bias of the original layers
            fake_quant.weight.data.copy_(linear.weight.data)

            if linear.bias is not None:
                fake_quant.bias.data.copy_(linear.bias.data)

            delattr(attn, linear_name)
            setattr(attn, linear_name, fake_quant)

    def freeze(self, attn):
        for linear_name in ("to_q", "to_k", "to_v"):
            linear = getattr(attn, linear_name)
            linear.freeze()
    ...
    def __call__(
        ...
        # `sample` projections.
        query = attn.to_q(hidden_states, timestep)
        key = attn.to_k(hidden_states, timestep)
        value = attn.to_v(hidden_states, timestep)
